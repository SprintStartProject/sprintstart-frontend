...
export function DragHandle({ visibleClassName, className = "" }: DragHandleProps) {
    return (
        <span
            className={`flex w-0 shrink-0 items-center justify-center overflow-hidden text-app-text-disabled opacity-0 transition-all cursor-grab active:cursor-grabbing ${visibleClassName} ${className}`.trim()}
            aria-hidden="true"
        >
