...
import type { ConnectState, SourceConnectMeta, SourceSystem } from "../types.ts";
...
export function SourceConnectModal({
  ...
  const isSubmitDisabled =
    isLoading || !isGithubSelected || !owner.trim() || !tokenName.trim() || !hasGithubTokens;
  ...
  return (
    ...
    >
      <form id="source-connect-form" onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="rounded-2xl border border-app-border bg-app-surface-muted p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-app-text-muted">{selectedMeta.description}</p>
            </div>
          </div>
        </div>
        ...
        {!isGithubSelected && (
          <div className="rounded-2xl border border-app-warning-border bg-app-warning-bg px-4 py-3 text-sm text-app-warning-text">
            {selectedMeta.type} can already be selected in the UI, but no frontend service is
            connected for this source type yet.
          </div>
        )}
        ...
        {isGithubSelected && (
          <>
            <div>
              <label htmlFor="github-owner" className="text-sm font-medium text-app-text">
                Repository owner
              </label>
            ...
            <div>
              <label htmlFor="github-repository-name" className="text-sm font-medium text-app-text">
                Repository name
              </label>
            ...
            <div>
              <label htmlFor="github-token-name" className="text-sm font-medium text-app-text">
                GitHub access token
              </label>
              ...
              {!hasGithubTokens && (
                <p className="mt-2 text-sm text-app-warning-text">
                  Add a GitHub personal access token first, then come back to connect a repository.
                </p>
              )}
            </div>
          </>
