...
export function SleepyBot({
  ...
  useEffect(() => {
    if (!isDizzy) return;
    const timer = window.setTimeout(() => setIsDizzy(false), DIZZY_DURATION_S * 1000);
  ...
  return (
    <span className={`relative inline-flex ${className ?? ""}`}>
      ...
      {asleep && (
        <span
          aria-hidden="true"
          className="pointer-events-none absolute top-0 left-full leading-none font-semibold text-app-brand-text select-none"
