...
export function KnowledgeBaseWidget() {
    ...
    useEffect(() => {
        ...
        async function loadArtifacts() {
            ...
            const data = await knowledgeService.getRecentArtifacts(
                projectId,
                PREVIEW_COUNT,
            );
    ...
    return (
        <div className="group relative flex min-h-56 flex-col overflow-hidden rounded-2xl border border-app-border bg-app-surface p-6 transition-colors hover:border-app-brand-border-strong">
            ...
            <div className="relative mb-5 flex items-center justify-between gap-2">
                ...
                <Link
                    to="/knowledge-base"
                    className="flex shrink-0 items-center gap-1 rounded-lg text-xs font-medium text-app-text-muted transition-colors hover:text-app-brand-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
                >
                    Browse
            ...
            {loading ? (
                ...
                <div className="flex flex-1 flex-col items-start justify-center gap-2">
                    <p className="text-sm text-app-text-muted">
                        Nothing here yet — sources appear once your project has
                        been ingested.
                    </p>
                </div>
            ) : (
                    ...
                    {artifacts.map((artifact) => (
                        <li key={artifact.id}>
                            ...
                            >
                                ...
                                <span className="shrink-0 text-[11px] tabular-nums text-app-text-muted">
