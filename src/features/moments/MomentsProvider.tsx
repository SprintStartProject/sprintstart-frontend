...
import type { Celebration, CelebrationInput, PathRevealHandlers } from "./types.ts";
...
export function MomentsProvider({ children }: { children: ReactNode }) {
  ...
  const revealPath = useCallback((handlers?: PathRevealHandlers) => {
    ...
    setPathReveal((current) => current ?? { id, onLaunched: handlers?.onLaunched });
    ...
    return () => setPathReveal((current) => (current?.id === id ? null : current));
  }, []);
  ...
  return (
    <MomentsContext.Provider value={value}>
      ...
      {pathReveal && (
        <PathReveal key={pathReveal.id} onLaunch={pathReveal.onLaunched} onDone={endPathReveal} />
      )}
