    ...
    useEffect(() => {
        ...
        const drawParticles = (w: World, palette: Palette) => {
            for (const pt of w.particles) {
                ...
                ctx.fillStyle =
                    pt.color === "alien" ? palette.dangerText : palette.brand;
                ctx.beginPath();
        ...
        const updatePlay = (w: World, dt: number) => {
            ...
            w.player.x = Math.max(
                0,
                Math.min(CANVAS_WIDTH - w.player.width, w.player.x),
            );
            ...
            w.alienSpeed = Math.min(
                ALIEN_BASE_SPEED + killed * ALIEN_SPEED_RAMP,
                ALIEN_SPEED_CAP,
            );
            ...
            for (const a of w.aliens) {
                ...
                for (let i = 0; i < w.lasers.length; i++) {
                    ...
                    if (
                        ...
                        const milestone =
                            Math.floor(w.scoreF / MILESTONE_STEP) * MILESTONE_STEP;
        ...
        >
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 flex items-center justify-between px-4 py-2.5">
                <div className="flex items-center gap-2">
                    <span className="rounded-md bg-app-surface/80 px-2 py-0.5 text-[11px] font-semibold tabular-nums text-app-text-muted backdrop-blur-sm">
                        HI {String(highScore).padStart(5, "0")}
                    </span>
                    <span className="rounded-md bg-app-surface/80 px-2 py-0.5 text-[11px] font-bold tabular-nums text-app-text backdrop-blur-sm">
