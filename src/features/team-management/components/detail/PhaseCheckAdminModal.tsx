...
function toPayload(drafts: QuestionDraft[]): UpsertPhaseCheckQuestion[] {
    return drafts.map((draft, index) => ({
        ...
        correctAnswer:
            draft.type === 'SHORT_TEXT' ? draft.correctAnswer.trim() : undefined,
        options:
...
export function PhaseCheckAdminModal({
    ...
    return (
        ...
        >
            <div
                role="tablist"
                aria-label="Knowledge check sections"
            ...
            {questions && tab === 'results' && (
                <ResultsTab attempts={attempts} questions={questions} />
            )}
    ...
    return (
        <div className="space-y-4">
            <p className="rounded-xl bg-app-surface-muted px-3 py-2 text-xs text-app-text-muted">
                Saving replaces this member&apos;s check for the phase. Earlier attempts stay as
                history, and questions they already cleared are not asked again.
            </p>
                ...
                <div key={draft.key} className="rounded-2xl border border-app-border p-4">
                    ...
                    {draft.type === 'MULTIPLE_CHOICE' ? (
                        ...
                        <input
                            ...
                            onChange={(event) =>
                                onUpdate(draft.key, { correctAnswer: event.target.value })
                            }
                    ...
                    <input
                        ...
                        onChange={(event) =>
                            onUpdate(draft.key, { explanation: event.target.value })
                        }
