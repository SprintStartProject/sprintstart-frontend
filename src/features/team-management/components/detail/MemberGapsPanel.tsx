...
export function MemberGapsPanel({
  ...
  const skillsByRole = skillLevels.reduce<Record<string, UserSkillLevel[]>>((acc, skill) => {
            ...
            {Object.entries(skillsByRole).map(([roleName, skills]) => (
              <div key={roleName}>
                ...
                <div className="space-y-1">
                  {skills.map((skill) => (
                    <SkillAssessmentRow key={skill.id} skill={skill} />
                  ))}
                </div>
              </div>
            ))}
...
function SkillAssessmentRow({ skill }: { skill: UserSkillLevel }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-app-border bg-app-surface-muted px-3 py-2">
      <span className="text-sm font-medium text-app-text">{skill.skillName}</span>
          ...
          knowledgeGaps.map((gap) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <p className="min-w-0 text-sm font-medium text-app-text">{gap.component}</p>
                <span
