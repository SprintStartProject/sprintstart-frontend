        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <p className="text-sm font-semibold text-app-text">{phase.title}</p>
              <span
  ...
  return (
    <div className="space-y-3">
      {steps.map((step, index) => {
        ...
        const timingDelta =
          actualMinutes && step.estimatedMinutes ? actualMinutes - step.estimatedMinutes : null;
        const isDragging = draggedStepId === step.id;
        const isDragTarget = dragOverStepId === step.id && draggedStepId !== step.id;
        ...
        return (
          <div
            ...
            onDrop={(event) => {
              ...
              onReorderSteps(selectedPhase.id, draggedStepId, step.id);
...
function StepCard({
  ...
  return (
    <div
      ...
      } ${step.status === "FINISHED" || step.status === "SKIPPED" ? "opacity-70" : ""} ${
      ...
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <span
              className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-medium ${getStepStatusStyles(step.status)}`}
            >
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-app-text-muted">
            ...
            <span className="rounded-full bg-app-surface-muted px-2 py-0.5">
              {taskCount ? `${taskCount.done}/${taskCount.total} tasks` : "Tasks ..."}
            </span>
            {actualMinutes && (
              <span
            ...
            {skipReason && (
              ...
              >
                ...
                {step.status === "SKIPPED" ? "Skipped" : "Skip requested"}
              </span>
            )}
          </div>
        </div>
...
function StepInsertButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
