 ...
 */
export function ProjectOverviewSummary({ projects, insights }: ProjectOverviewSummaryProps) {
  const loaded = projects
    .map((project) => insights[project.id])
    .filter((entry): entry is ProjectInsights => Boolean(entry));

  const memberCount = projects.reduce((sum, project) => sum + project.users.length, 0);

  const attentionLoaded = loaded.filter((entry) => entry.attention !== null);
  const waitingCount = attentionLoaded.reduce(
    (sum, entry) =>
      sum +
      (entry.attention ?? []).filter((item) => item.hasOpenSkipRequest || item.hasUnreadFeedback)
        .length,
    0,
  );
  const stalledCount = attentionLoaded.reduce(
    (sum, entry) =>
      sum +
      (entry.attention ?? []).filter((item) => item.daysOnCurrentStep > STALLED_AFTER_DAYS).length,
    0,
  );
  ...
  const averageProgress =
    progressEntries.length > 0
      ? progressEntries.reduce((sum, value) => sum + value, 0) / progressEntries.length
      : null;

  const sourceEntries = loaded.filter((entry) => entry.sources !== null);
  const failedSources = sourceEntries.reduce(
    (sum, entry) =>
      sum + (entry.sources ?? []).filter((source) => source.connectionStatus === "FAILED").length,
    0,
  );
      ...
      <dl className="flex flex-wrap items-center gap-x-6 gap-y-2">
        {stats.map(({ key, icon: Icon, value, label, isAlert }) => (
          <div key={key} className="flex items-center gap-2">
            <Icon
              className={`h-4 w-4 ${isAlert ? "text-app-warning-text" : "text-app-text-muted"}`}
              aria-hidden="true"
            />
            <dd
              className={`text-lg font-semibold tabular-nums ${
                isAlert ? "text-app-warning-text" : "text-app-text"
              }`}
            >
              {value}
            </dd>
