...
export function ProjectManagementTab({ projects }: ProjectManagementTabProps) {
  ...
  const load = useCallback(async () => {
    ...
    const loaded = await Promise.all(
      projectIds.split(",").map((projectId) => projectService.getAccessibleProject(projectId)),
    );
  ...
  useEffect(() => {
    ...
    async function loadInitialData() {
      try {
        ...
        if (!active) return;
        setLoadError(error instanceof ApiError ? error.message : "Projects could not be loaded.");
      } finally {
  ...
  const orderedProjects = useMemo(
    () =>
      [...details].sort((a, b) => b.users.length - a.users.length || a.name.localeCompare(b.name)),
    [details],
  );

  const requestMove = useCallback(
    (user: ProjectUser, sourceProject: AdminProjectDetails, targetProjectId: string) => {
      if (targetProjectId === sourceProject.id) return;

      const targetProject = details.find((project) => project.id === targetProjectId);
  ...
  const confirmMove = useCallback(async () => {
    ...
    try {
      await projectService.transferProjectUser(pendingMove.targetProject.id, {
      ...
      setMoveError(error instanceof ApiError ? error.message : "The member could not be moved.");
    } finally {
      setMoving(false);
    }
  }, [load, pendingMove]);
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-app-border bg-app-surface p-8 text-center">
        <p className="text-sm text-app-text-muted">Loading your projects...</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="flex min-w-0 flex-col gap-4">
      <ProjectOverviewSummary projects={orderedProjects} insights={insights} />
      ...
      {orderedProjects.map((project) => {
        ...
        const managerRoles =
          project.users.find((user) => user.id === managerId)?.projectRoles ?? [];
        const rolesInUse = [...new Set(project.users.flatMap((user) => user.projectRoles))].sort();
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div className="min-w-0">
                {project.manager && (
                  <div className="mb-2 flex flex-col gap-2 rounded-xl border border-app-brand-border bg-app-brand-soft px-3 py-2 sm:flex-row sm:items-center sm:gap-3">
                    ...
                    <Badge variant="brand" className="self-start sm:shrink-0">
                      <Crown className="mr-1.5 h-3 w-3" aria-hidden="true" />
                      Project manager
                    </Badge>
                  </div>
                )}
                      ...
                      >
                        ...
                        <FilterSelect
                          ...
                          onChange={(targetProjectId) =>
                            requestMove(user, project, targetProjectId)
                          }
                          className="sm:w-48"
                        />
                      </li>
              ...
              <dl className="flex min-w-0 flex-col gap-2.5 lg:border-l lg:border-app-border lg:pl-4">
                ...
                <ProjectInsightsPanel insights={insights[project.id] ?? EMPTY_INSIGHTS} />
              </dl>
            </div>
          </section>
        );
      ...
      <AlertDialog
        ...
        description={
          pendingMove ? (
            <>
              <strong>{fullName(pendingMove.user)}</strong> moves from{" "}
              <strong>{pendingMove.sourceProject.name}</strong> to{" "}
              <strong>{pendingMove.targetProject.name}</strong>. Their project roles move along.
            </>
          ) : undefined
        }
        confirmLabel="Move"
...
function fullName(person: { firstName: string; lastName: string; username: string }): string {
