  ...
  return (
    ...
    >
      <button
        ...
        className={`absolute inset-0 focus-visible:ring-2 focus-visible:ring-app-focus focus-visible:outline-none ${
      ...
      <div className="pointer-events-none relative flex w-full min-w-0 items-center gap-2">
        ...
        {compact && (
          <span
            className={`inline-flex min-w-5 shrink-0 items-center justify-center rounded-full px-1.5 py-0.5 text-[11px] leading-none font-bold tabular-nums ${
              selected ? "bg-app-brand/20 text-app-brand-text" : "bg-app-bg text-app-text-subtle"
            }`}
          >
            {members.length}
          </span>
      ...
      {!compact && (
        <div className="pointer-events-none relative flex w-full min-w-0 flex-col">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              {shownMembers.map((member) => {
                ...
                return (
                  <span
                    key={member.userId}
                    className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-app-border bg-app-bg py-0.5 pr-2 pl-0.5"
                  >
                    <UserAvatar
          ...
          {role.description && (
            <p className="mt-2 line-clamp-2 text-xs text-app-text-muted">{role.description}</p>
          )}

          <div
            className={`mt-3 flex flex-wrap gap-1.5 ${
              hasSkillsDivider ? "border-t border-app-border pt-3" : ""
            }`}
          >
            {skills.map((skill) => (
              <span
              ...
              >
                ...
                {skill.status === "RETIRED" && <span className="font-medium">Retired</span>}
              </span>
            ))}

            {skills.length === 0 && <p className="text-xs text-app-text-muted">No skills yet.</p>}
          </div>
        </div>
      )}
    </motion.div>
