...
describe("RunHistory", () => {
  ...
  it("renders the empty state when no runs are provided", () => {
    ...
    expect(
      screen.getByText("The backend did not return any ingestion runs yet."),
    ).toBeInTheDocument();
  });
  ...
  it("renders a Success status badge for a completed run", () => {
    const runs: IngestionRun[] = [createMockRun({ status: "COMPLETED" })];
  ...
  it("renders a Running status badge for an in-progress run", () => {
    const runs: IngestionRun[] = [createMockRun({ status: "RUNNING", finishedAt: null })];
  ...
  it("renders a Failed status badge for a failed run", () => {
    const runs: IngestionRun[] = [createMockRun({ status: "FAILED" })];
  ...
  it("renders a Partial status badge for a partial run", () => {
    const runs: IngestionRun[] = [createMockRun({ status: "PARTIAL" })];
  ...
  it("exposes the run-level failure reason on the failed status badge", () => {
    render(
      <RunHistory runs={[createMockRun({ status: "FAILED", failureReason: "Token expired" })]} />,
    );
  ...
  it("renders In progress for a running run with no finishedAt", () => {
    const runs: IngestionRun[] = [createMockRun({ finishedAt: null, status: "RUNNING" })];
  ...
  it("shows an Indexing badge alongside Success when the AI sync is still pending", () => {
    const runs: IngestionRun[] = [createMockRun({ status: "COMPLETED", aiSyncStatus: "PENDING" })];
