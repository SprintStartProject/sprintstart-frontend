...
describe("AccountEnabledToggle", () => {
  ...
  it("renders a switch role with an accessible label", () => {
    ...
    expect(screen.getByRole("switch", { name: "Toggle account access" })).toBeInTheDocument();
  });
  ...
  it("calls onChange with the inverted value when clicked", async () => {
    ...
    render(<AccountEnabledToggle {...defaultProps} enabled={false} onChange={onChange} />);
  ...
  it("calls onChange with false when clicking an enabled toggle", async () => {
    const onChange = vi.fn();
    const user = userEvent.setup();
    render(<AccountEnabledToggle {...defaultProps} enabled={true} onChange={onChange} />);
  ...
  it("does not fire onChange and renders disabled when disabled is true", async () => {
    const onChange = vi.fn();
    const user = userEvent.setup();
    render(<AccountEnabledToggle {...defaultProps} disabled={true} onChange={onChange} />);
