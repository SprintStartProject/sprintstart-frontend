...
describe("TokensTab", () => {
  ...
  it("adds a new token and refreshes the list", async () => {
    ...
    await waitFor(() => {
      expect(githubServiceMock.addGithubPat).toHaveBeenCalledWith("default", "ghp_test");
  ...
  it("rotates an existing token", async () => {
    ...
    await waitFor(() => {
      expect(githubServiceMock.updateGithubPat).toHaveBeenCalledWith("default", "github_pat_new");
      expect(onRefresh).toHaveBeenCalled();
    });
  });
  ...
  it("deletes an existing token after inline confirmation", async () => {
    ...
    const confirmation = screen.getByText(/This cannot be undone/).closest("div");
    ...
    await waitFor(() => {
      expect(githubServiceMock.deleteGithubPat).toHaveBeenCalledWith("default");
      expect(onRefresh).toHaveBeenCalled();
    });
  });
});
