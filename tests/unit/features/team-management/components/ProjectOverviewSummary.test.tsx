...
import type { AdminProjectDetails, ProjectUser } from "../../../../../src/services/projectService";
...
describe("ProjectOverviewSummary", () => {
  it("adds the figures up across every project", () => {
    render(<ProjectOverviewSummary projects={projects} insights={insights} />);
    ...
    expect(within(summary()).getByText("sources failed")).toBeInTheDocument();
  });
  ...
  it("hides the problem counters when there is no problem", () => {
    ...
    expect(within(summary()).queryByText(/sources? failed/)).not.toBeInTheDocument();
    expect(within(summary()).queryByText(/on one step/)).not.toBeInTheDocument();
    expect(within(summary()).getByText("waiting on you")).toBeInTheDocument();
  });
  ...
  it("leaves out counters whose data never arrived", () => {
    ...
    expect(within(summary()).queryByText("waiting on you")).not.toBeInTheDocument();
    expect(within(summary()).queryByText("average onboarding")).not.toBeInTheDocument();
    expect(within(summary()).getByText("members")).toBeInTheDocument();
  });
});
