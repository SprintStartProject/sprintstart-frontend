...
function projectUser(id: string, firstName: string, projectRoles: string[] = []): ProjectUser {
  return {
    id,
...
describe("ProjectManagementTab", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(projectService.getAccessibleProject).mockImplementation((projectId: string) =>
  ...
  it("groups members under their own project", async () => {
    ...
    expect(within(alphaSection).getByText("Max Mustermann")).toBeInTheDocument();
    expect(within(betaSection).getByText("Lena Mustermann")).toBeInTheDocument();
    expect(within(alphaSection).queryByText("Lena Mustermann")).not.toBeInTheDocument();
  });
  ...
  it("shows the project description, roles and connected sources", async () => {
    ...
    expect(within(alphaSection).getByText("The alpha project")).toBeInTheDocument();
    ...
    const betaSection = screen.getByRole("region", { name: "Beta" });
    expect(within(betaSection).getByText("Nothing connected")).toBeInTheDocument();
  });
  ...
  it("marks the manager and gives them no move control", async () => {
    ...
    expect(within(alphaSection).getByText("Project manager")).toBeInTheDocument();
    expect(within(alphaSection).getByText("Erika Mustermann")).toBeInTheDocument();
    expect(
  ...
  it("moves a member into the picked project after confirmation", async () => {
    ...
    await waitFor(() => {
      expect(projectService.transferProjectUser).toHaveBeenCalledWith("p2", {
        userId: "u1",
        sourceProjectId: "p1",
      });
    });
  ...
  it("keeps the dialog open and shows why a move was refused", async () => {
    ...
    expect(
      await screen.findByText("User with id u1 manages project with id p1."),
    ).toBeInTheDocument();
  });
});
