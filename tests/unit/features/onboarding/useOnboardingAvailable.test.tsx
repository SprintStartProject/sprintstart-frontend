...
describe('useOnboardingAvailable', () => {
    ...
    it('is available once a role is held and the project has content', async () => {
        ...
        await waitFor(() =>
            expect(knowledgeService.hasIngestedContent).toHaveBeenCalledWith('p1'),
        );
    ...
    it('stays visible when the content check fails, rather than hiding navigation', async () => {
        ...
        vi.mocked(knowledgeService.hasIngestedContent).mockRejectedValue(
        ...
        await waitFor(() =>
            expect(knowledgeService.hasIngestedContent).toHaveBeenCalled(),
        );
    ...
    it('stays visible while the answer is still in flight', () => {
        vi.mocked(knowledgeService.hasIngestedContent).mockReturnValue(
            new Promise(() => {}),
        );
