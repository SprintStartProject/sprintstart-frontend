...
const projects: ProjectOverview[] = [
  {
    ...
    users: [{ id: "u1", username: "asmith", email: "alice@example.com", projectRoles: ["MEMBER"] }],
  },
  {
    id: "p2",
    name: "Backend",
...
describe("ProjectsTab Accessibility", () => {
  it("should not have any a11y violations", async () => {
    const { baseElement } = render(
      <MemoryRouter>
        <main>
          <ProjectsTab filteredProjects={projects} onOpenProjectDetails={vi.fn()} />
        </main>
      </MemoryRouter>,
    );

    expect(
      screen.getByRole("button", { name: "Open details for SprintStart" }),
    ).toBeInTheDocument();
