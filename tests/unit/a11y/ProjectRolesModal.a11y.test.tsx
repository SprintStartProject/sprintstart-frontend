...
vi.mock("../../../src/services/teamManagementService", () => ({
  getProjectRoles: vi
    .fn()
    .mockResolvedValue([{ id: "r1", name: "Developer", description: "Builds features" }]),
  getSkills: vi
    .fn()
    .mockResolvedValue([{ id: "sk1", name: "React", roleIds: ["r1"], status: "ACTIVE" }]),
  createProjectRole: vi.fn().mockResolvedValue({ id: "r2", name: "Designer", description: "" }),
  createSkill: vi
    .fn()
    .mockResolvedValue({ id: "sk2", name: "Figma", roleIds: ["r2"], status: "ACTIVE" }),
...
function ProjectRolesModalHarness() {
  ...
  return (
    <main>
      ...
      <ProjectRolesModal open={isOpen} onClose={() => setIsOpen(false)} />
    </main>
  );
}
